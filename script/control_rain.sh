...
if [ $epoch_rain_start -gt 0 ]; then
  ...
  if [ [ $(($epoch_rain_start-$spot_timeout)) -gt $epoch_current ] && [ $(($epoch_rain_stop+$spot_timeout)) -lt $epoch_current ] ]; then
    echo "It will rain or has been raining within the next $spot_timeout seconds, to turn off the spots"
    switchrelais[${relaisnamepin["Spot Stein"]}]=0
    switchrelais[${relaisnamepin["Spot Ecke"]}]=0
  fi
  if [ [ $epoch_rain_start -ge $epoch_current ] && [ $epoch_rain_stop -le $epoch_current ] ]; then
